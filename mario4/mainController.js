mario.initialize = function() {
    var level = prompt("Plz choose level(1, 2 or 3)", "1");
    switch (+level) {
        case 1:
            break;
        case 2:
    ...
    mario.mapController.initialize(mario.canvasController.ctxs[1].canvas, level, () => {
...
//mapController
mario.mapController = {
    initialize: function(canvas, level, callback) {
        this.canvas = canvas;
        this.retrieveMapData(level, callback);
    },
    retrieveMapData: function(level, callback) {
        ...
        xml.open('get', mario.url + 'mario/json/' + level, true);
        xml.send();
    }
};
